...
// Inline close icon — avoids @untitledui/icons dependency
const CloseX = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
...
const withBadgeTypes = withPillTypes;
...
interface BadgeProps<T extends BadgeTypes> {
    type?: T | "badge-modern" | "badge-color";
    size?: Sizes;
    color?: BadgeColor<T> | string;
    children: ReactNode;
    className?: string;
}
...
export const Badge = <T extends BadgeTypes>(props: BadgeProps<T>) => {
    ...
    const colors = withPillTypes[type as keyof typeof withPillTypes] || withPillTypes[badgeTypes.pillColor];
    const styleObj = (colors.styles as any)?.[color] || (colors.styles as any)?.gray || filledColors[color as BadgeColors] || filledColors.gray;
    ...
    const sizes: Record<string, typeof pillSizes> = {
    ...
    const sizeClass = sizes[type]?.[size] || pillSizes[size] || pillSizes.md;

    return <span className={cx(colors.common, sizeClass, styleObj?.root, props.className)}>{children}</span>;
};

interface BadgeWithDotProps<T extends BadgeTypes> {
    type?: T | "badge-modern" | "badge-color";
    size?: Sizes;
    color?: BadgeTypeToColorMap<typeof withBadgeTypes>[T] | string;
    className?: string;
    children: ReactNode;
}
...
export const BadgeWithDot = <T extends BadgeTypes>(props: BadgeWithDotProps<T>) => {
    ...
    const colors = withBadgeTypes[type as keyof typeof withBadgeTypes] || withBadgeTypes[badgeTypes.pillColor];
    const styleObj = (colors.styles as any)?.[color] || (colors.styles as any)?.gray || filledColors[color as BadgeColors] || filledColors.gray;
    ...
    return (
        <span className={cx(colors.common, sizeClass, styleObj?.root, className)}>
            <Dot className={styleObj?.addon} size="sm" />
            {children}
        </span>
    );
};
...
export const BadgeWithIcon = <T extends BadgeTypes>(props: BadgeWithIconProps<T>) => {
    ...
    const badgeSizes = {
        ...
        md: {
            trailing: "gap-1 py-0.5 pl-2 pr-1 text-sm font-medium",
    ...
    const sizeClass = sizes[type]?.[size]?.[icon] || pillSizes[size]?.[icon] || pillSizes.md.trailing;

    return (
        <span className={cx(colors.common, sizeClass, styleObj?.root, className)}>
            {IconLeading && <IconLeading className={cx(styleObj?.addon, "size-3 stroke-3")} />}
            {children}
            {IconTrailing && <IconTrailing className={cx(styleObj?.addon, "size-3 stroke-3")} />}
        </span>
    );
};
...
interface BadgeWithFlagProps<T extends BadgeTypes> {
    ...
    size?: Sizes;
    flag?: FlagTypes;
    color?: BadgeTypeToColorMap<typeof withPillTypes>[T] | string;
    children: ReactNode;
}
...
export const BadgeWithFlag = <T extends BadgeTypes>(props: BadgeWithFlagProps<T>) => {
    ...
    return (
        <span className={cx(colors.common, sizeClass, styleObj?.root)}>
...
export const BadgeIcon = <T extends BadgeTypes>(props: BadgeIconProps<T>) => {
    ...
    return (
        <span className={cx(colors.common, sizeClass, styleObj?.root)}>
            <Icon className={cx("size-3 stroke-[3px]", styleObj?.addon)} />
        </span>
    );
};
